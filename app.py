...
BODACC_URL = "https://bodacc-datadila.opendatasoft.com/api/explore/v2.1/catalog/datasets/annonces-commerciales_qualif/records"
CODES = {
...
def refresh_france():
    try:
        ...
        params = {
            ...
            "where": "familleavis='collective' and dateparution>=date'" + cutoff_str + "'"
        }
        r = requests.get(BODACC_URL, params=params, headers=HEADERS, timeout=15)
        ...
        for item in results:
            try:
                ...
                if date_str and len(date_str) == 8:
                    notice_date = date_str[:4] + "-" + date_str[4:6] + "-" + date_str[6:]
                else:
                    notice_date = date_str
                jugement = item.get("jugement", "{}")
                if isinstance(jugement, str):
                    jugement = json.loads(jugement) if jugement else {}
                nature = jugement.get("nature", "Insolvency Proceedings") if jugement else "Insolvency Proceedings"
                url = "https://www.bodacc.fr" + item.get("url", "")
                cur.execute(
                    "INSERT INTO insolvencies VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (id) DO NOTHING",
