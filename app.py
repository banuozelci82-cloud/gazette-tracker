...
def refresh():
    ...
    return jsonify({"status": "ok", "new_uk": new_uk, "new_us": total_us, "new_fr": new_fr, "new_us_public": new_us_public, "new_us_nonpublic": new_us_nonpublic})
...
def refresh_france():
    try:
        ...
        params = {
            ...
            "where": "familleavis='collective' and dateparution>='" + cutoff.replace("-", "") + "'"
        }
        ...
        for item in results:
            try:
                ...
                if date_str and len(date_str) == 8:
                    ...
                    notice_date = item.get("dateparution", "")
                jugement = item.get("jugement", "{}")
                if isinstance(jugement, str):
                    jugement = json.loads(jugement) if jugement else {}
                nature = jugement.get("nature", "Insolvency") if jugement else "Insolvency"
                url = "https://www.bodacc.fr" + item.get("url", "")
                cur.execute(
                    "INSERT INTO insolvencies VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (id) DO NOTHING",
                    (notice_id, company_name, nature,
                     url,
                     datetime.now().strftime("%Y-%m-%d %H:%M"),
                     notice_date, item.get("registre", [""])[0] if item.get("registre") else "", "", "FR")
                )
                if cur.rowcount > 0:
                    new += 1
            except Exception as e:
