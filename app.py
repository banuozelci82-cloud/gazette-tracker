...
import requests, csv, io, os, openpyxl, re, pdfplumber, json
...
def debug_fr():
    try:
        params = {"limit": 5, "order_by": "dateparution desc", "where": "familleavis='collective'"}
        ...
        return jsonify({
            ...
            "sample": [{"id": item.get("id"), "date": item.get("dateparution"), "name": item.get("commercant")} for item in results]
        })
...
FRENCH_TYPES = {
    "Jugement d'ouverture de liquidation judiciaire": "Judicial Liquidation",
    "Jugement d'ouverture de redressement judiciaire": "Administration/Restructuring",
    "Jugement d'ouverture de sauvegarde": "Safeguard Procedure",
    "Jugement de conversion en liquidation judiciaire": "Conversion to Liquidation",
    "Jugement de cloture pour insuffisance d'actif": "Closure - Insufficient Assets",
    "Jugement de cloture de liquidation": "Liquidation Closure",
    "Autre jugement et ordonnance": "Court Order",
    "Depot de l'etat des creances": "Creditors Statement",
    "Plan de redressement": "Restructuring Plan",
    "Plan de cession": "Asset Sale Plan",
}
...
def refresh_france():
    try:
        ...
        for item in results:
            try:
                listepersonnes = item.get("listepersonnes", "{}")
                if isinstance(listepersonnes, str):
                    listepersonnes = json.loads(listepersonnes)
                ...
                if type_personne != "pm":
                    continue
                company_name = personne.get("denomination", item.get("commercant", ""))
                if not company_name:
                    continue
                ...
                if isinstance(jugement, str):
                    jugement = json.loads(jugement) if jugement else {}
                nature_fr = jugement.get("nature", "Insolvency Proceedings") if jugement else "Insolvency Proceedings"
                nature = FRENCH_TYPES.get(nature_fr, nature_fr)
                url = "https://www.bodacc.fr/pages/annonces-commerciales-detail/?q.id=id:" + item.get("id", "")
                cur.execute(
                    "INSERT INTO insolvencies VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (id) DO NOTHING",
                    (notice_id, company_name, nature, url,
                     datetime.now().strftime("%Y-%m-%d %H:%M"),
                     notice_date,
                     item.get("registre", [""])[0] if item.get("registre") else "",
                     "", "FR")
                )
                if cur.rowcount > 0:
                    new += 1
            except Exception as e:
                print("FR insert error: " + str(e))
                conn.rollback()
        conn.commit()
        conn.close()
        return new
    except Exception as e:
        print("FR refresh error: " + str(e))
        return 0
