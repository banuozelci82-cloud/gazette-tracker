...
def debug_us():
    ...
    url = "https://www.courtlistener.com/api/rest/v4/search/?type=d&q=LLC+OR+Inc+OR+Corp+OR+Limited+chapter+11&order_by=dateFiled+desc&filed_after=" + cutoff + "&page_size=10"
    r = requests.get(url, headers=cl_headers, timeout=15)
    data = r.json()
    results = data.get("results", [])
    return jsonify({"count": data.get("count", 0), "results": [{"name": item.get("caseName"), "chapter": item.get("chapter"), "date": item.get("dateFiled"), "court": item.get("court_id")} for item in results]})
